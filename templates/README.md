...
- **Claude login:** if you have a `CLAUDE_CODE_OAUTH_TOKEN` exported in your host shell (mint one on your machine with `claude setup-token`), `npm run claude` passes it straight through to the workspace and Claude is logged in automatically — no `/login` step. The token is passed by name (`docker compose exec -e CLAUDE_CODE_OAUTH_TOKEN`), so its value never lands on the command line. Otherwise, run `npm run claude` and use `/login` once; that login persists in `workspace/` across rebuilds.
